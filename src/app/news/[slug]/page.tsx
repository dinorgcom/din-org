...
export function generateStaticParams() {
  return editorialArticles.en.map(({ slug }) => ({ slug }));
}
...
export async function generateMetadata({
  ...
  const article = getEditorialArticle("en", slug);
  if (!article) return {};
  const canonical = `https://din.org/news/${article.slug}`;
  const german = `https://din.org/de/news/${article.alternateSlug}`;
  return {
    ...
    alternates: {
      canonical,
      languages: { en: canonical, de: german, "x-default": canonical },
    },
    openGraph: {
      type: "article",
      locale: "en_US",
...
export default async function ArticlePage({
  ...
  const article = getEditorialArticle("en", slug);
  if (!article) notFound();
  const url = `https://din.org/news/${article.slug}`;
  const jsonLd = {
